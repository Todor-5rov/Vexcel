...
export default function ChatInterface({
  ...
  // 🔥 CRITICAL: Memoize these values to prevent infinite re-renders
  const dataLength = useMemo(() => currentData.length, [currentData.length])
  ...
  // 🔥 CRITICAL: Memoize the loadChatSession function
  const loadChatSession = useCallback(async () => {
    console.log("Loading chat session for:", { fileId, userId, mcpFilePath })
    ...
    if (isLoadingChat || chatInitialized) {
      console.log("Chat already loading or initialized, skipping...")
      return
    }
    ...
    try {
      ...
      if (existingMessages.length === 0) {
        ...
        const welcomeMessage = ChatService.createWelcomeMessage(filename, dataLength, headers)

        // Save welcome message if we have a real session
        if (!newSessionId.startsWith("temp-")) {
      ...
      setChatInitialized(true) // 🔥 CRITICAL: Mark as initialized
    } catch (error) {
  ...
  }, [fileId, userId, mcpFilePath, dataLength, headersString, isLoadingChat, chatInitialized])

  // 🔥 CRITICAL: Only load chat session when key values change AND not already initialized
  useEffect(() => {
    if (!chatInitialized) {
      loadChatSession()
    }
  }, [loadChatSession, chatInitialized])

  // 🔥 CRITICAL: Reset initialization when file changes
  useEffect(() => {
